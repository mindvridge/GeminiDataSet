...
def _get_session_structure(min_turns: int, max_turns: int) -> str:
    """
    턴 수에 따른 세션 구조 지침 동적 생성
    ...
    Returns:
        세션 구조 지침 문자열
    """
    # short (5-10턴): 간단한 4단계 구조
    if max_turns <= 10:
        return """
### 세션 구조 가이드 (5-10턴)
다음 단계를 따라 상담을 진행하세요:

1. **도입 단계 (1-2턴)**: 라포 형성, 주호소 파악
2. **탐색 단계 (3-5턴)**: 문제 깊이 탐색, 감정 반영
3. **작업 단계 (6-8턴)**: 통찰 유도, 대안 모색
4. **마무리 단계 (9-10턴)**: 요약, 과제 부여, 다음 상담 예고
"""

    # medium (15-25턴): 확장된 5단계 구조
    elif max_turns <= 25:
        return f"""
### 세션 구조 가이드 (15-25턴) - 반드시 {min_turns}턴 이상!
다음 단계를 따라 충분히 깊은 상담을 진행하세요:
...
⚠️ 주의: 탐색 단계와 심화 탐색 단계에서 충분한 시간을 들여 문제를 깊이 탐색하세요.
급하게 마무리하지 말고, 내담자의 이야기를 충분히 들어주세요.
"""

    # long (30-40턴): 포괄적인 7단계 구조
    else:
        return f"""
### 세션 구조 가이드 (30-40턴) - 반드시 {min_turns}턴 이상!
다음 단계를 따라 심층적인 상담을 진행하세요:
...
⚠️ 중요 주의사항:
- 각 단계를 서두르지 않고 충분히 진행하세요
- 특히 탐색 단계(5-16턴)와 통찰 촉진 단계(17-22턴)에서 깊이 있는 대화를 나누세요
- 내담자의 감정과 경험을 충분히 탐색한 후에 작업 단계로 넘어가세요
- 30턴 미만으로 종료하면 안 됩니다. 반드시 {min_turns}턴 이상 생성하세요!
"""
...
def get_scenario_prompt(
    ...
    # 턴 수에 따른 세션 구조 동적 생성
    session_structure = _get_session_structure(min_turns, max_turns)
...
⚠️ **필수**: 반드시 {min_turns}턴 이상의 대화를 생성해야 합니다. 조기 종결하지 마세요!
